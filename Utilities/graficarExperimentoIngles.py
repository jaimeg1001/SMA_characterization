...
def process_sma_data():
    ...
    folder_path = "Test_bueno\\20250606_131214"  # Carpeta base
    ...
    if experiment_type == "fuerza_constante":
        ...
        titulo = f"Experiment - Constant Force ({valor_aplicado:.3f} {unidad_aplicada})"
        
        if not calentamiento_procesado.empty:
            plt.plot(calentamiento_procesado['temperature'], 
                    calentamiento_procesado['deflexion_corregida_mm'], 
                    'ro-', label='Heating', linewidth=2, markersize=6)
        
        if not enfriamiento_procesado.empty:
            plt.plot(enfriamiento_procesado['temperature'], 
                    enfriamiento_procesado['deflexion_corregida_mm'], 
                    'bo-', label='Cooling', linewidth=2, markersize=6)
    
    else:  # deflexion_constante
        # Graficar temperatura vs fuerza
        y_label = "Output Force (N)"
        titulo = f"Experiment - Constant Deflection ({valor_aplicado} {unidad_aplicada})"
    ...
    # Guardar la gráfica
    graph_filename = f"sma_grafica_{experiment_type}_english.png"
